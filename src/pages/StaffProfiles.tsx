...
export default function StaffProfiles() {
  ...
  const handleEdit = (person: Staff) => {
    ...
    // Pre-populate states and cities
    const country = countries.find(c => c.name === person.country);
    if (country) {
      const statesList = State.getStatesOfCountry(country.isoCode);
      setStates(statesList);
      const state = statesList.find(s => s.name === person.state);
      if (state) {
          ...
          >
            <div className="absolute top-4 right-4 z-20 flex gap-2">
              <button 
                ...
                className="p-3 bg-accent text-black rounded-2xl hover:scale-110 transition-all shadow-xl border border-accent/30 flex items-center justify-center"
                title="Edit Staff"
              >
                <Edit size={20} />
              </button>
              <button 
                ...
                className="p-3 bg-red-600 text-white rounded-2xl hover:bg-red-700 transition-all shadow-xl border border-red-400 flex items-center justify-center"
                title="Delete Staff"
              >
                <Trash2 size={20} />
              </button>
              ...
              <div className="h-64 bg-gradient-to-br from-accent/20 via-blue-500/10 to-purple-500/20 relative">
                <div className="absolute top-6 right-6 flex gap-3 z-10">
                  <button 
                    ...
                    className="p-3 bg-accent text-black hover:scale-110 rounded-full transition-all shadow-xl"
                    title="Edit Profile"
                  >
                    <Edit size={24} />
                  </button>
                  <button 
                    onClick={() => setSelectedStaff(null)}
                    className="p-3 bg-black/20 hover:bg-black/40 rounded-full transition-all"
                  >
                    <X size={24} />
                  </button>
                </div>
              ...
              <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 grid md:grid-cols-3 gap-4 pt-4 border-t border-glass-border">
                  <div className="space-y-1">
                    ...
                    <select 
                      required 
                      className="w-full bg-[#1a1a1a] border border-glass-border rounded-xl p-3 outline-none focus:border-accent/50 appearance-none text-white"
