...
app.put("/api/staff/:id", (req, res) => {
  ...
  try {
    const info = db.prepare(`
      UPDATE staff SET 
        name = ?, email = ?, mobile = ?, degree = ?, skills = ?, 
        languages = ?, father_name = ?, mother_name = ?, country = ?, 
        state = ?, city = ?, address = ?, pincode = ?, image = ?
      WHERE id = ?
    `).run(s.name, s.email, s.mobile, s.degree, s.skills, s.languages, s.father_name, s.mother_name, s.country, s.state, s.city, s.address, s.pincode, s.image, id);
